...
describe('TextInput component', () => {
  ...
  it('should get the wrapper info', async () => {
    const {getByTestId} = render(
      <BaseProvider>
    ...
    await act(async () => {
      fireEvent(wrapper, 'layout', event)
    })

    expect(wrapper.props.style.borderColor).toBe('#454545')
  })

  it('should get the wrapper info text input flat', async () => {
    const {getByTestId} = render(
      <BaseProvider>
  ...
  it('should get the label info', async () => {
    const {getByTestId} = render(
      <BaseProvider>
        <TextInput.Outlined value="" label="asd" />
      </BaseProvider>,
    )
    const label = getByTestId('test-Label')
    const event = {
    ...
    await act(async () => {
      fireEvent(label, 'layout', event)
    })

    expect(label.props.style.left).toBe(0)
  })

  it('should get the label info text input flat', async () => {
    const {getByTestId} = render(
      <BaseProvider>
        <TextInput.Flat value="" label="asd" />
      </BaseProvider>,
    )
    const label = getByTestId('test-Label')
    const event = {
