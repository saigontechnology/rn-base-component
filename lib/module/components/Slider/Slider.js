...
import SliderFixedRange from './SliderFixedRange';
...
import { DEFAULT_MAXIMUM_VALUE, DEFAULT_MINIMUM_VALUE, DEFAULT_STEP, DURATION, FIRST_POINT, INIT_POINT, INIT_VALUE, INVISIBLE, MINIMUM_TRACK_WIDTH, VISIBLE } from './constants';
...
const Slider = _ref => {
  ...
  const sliderInfo = useSharedValue({
    range: INIT_VALUE,
    trackWidth: INIT_VALUE
  });
  const sliderValue = useSharedValue(INIT_POINT);
  const progress = useSharedValue(INIT_VALUE);
   ...
   */
  const updateSlider = useCallback((progressing, value) => {
   ...
   */
  const onPressPoint = useCallback(pointIndex => {
