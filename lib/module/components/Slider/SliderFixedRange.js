...
import { DEFAULT_MAXIMUM_VALUE, DEFAULT_MINIMUM_VALUE, DEFAULT_STEP, DURATION, FIRST_POINT, INIT_POINT, INIT_VALUE, INVISIBLE, MINIMUM_TRACK_WIDTH, NEXT_STEP, PREVIOUS_STEP, THUMB_POSITION, VISIBLE } from './constants';
...
const SliderFixedRange = _ref => {
  ...
  const leftThumbRangeStyle = useAnimatedStyle(() => ({
    transform: [{
      translateX: withTiming(leftProgress.value, {
        duration: 1
      })
    }],
  ...
  const rightThumbRangeStyle = useAnimatedStyle(() => ({
    transform: [{
      translateX: withTiming(rightProgress.value - actualThumbSize.width / 2, {
        duration: 1
      })
    }],
